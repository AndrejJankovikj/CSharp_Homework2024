...
string carWithMaxHp = CarsData.Cars.OrderByDescending(car => car.HorsePower).Select(car => car.Model).FirstOrDefault();
...
ConsoleHelper.PrintInColor($"The highest MPG is {highestCarMPG}", ConsoleColor.DarkMagenta);
ConsoleHelper.PrintInColor($" The lowers MPG is {lowestCarMPG}", ConsoleColor.Blue);
